...
def widget(translate, lang):
    ...
    capital = st.number_input(translate["capital"][lang], 0, None, 0)

    e = np.arange(0.01, 100.0, 0.01)

    # Create functions to plot
    s = np.divide((np.log(salary) - np.log(capital * rate /
                                           100 + e * salary / 100)), np.log(1 + rate / 100))
    t = np.divide(- np.log(e / 100), np.log(1 + rate / 100))
    time = np.divide((np.log(salary) - np.log(capital * rate /
                                              100 + savings * salary / 100)), np.log(1 + rate / 100))
